...
import (
	...
	"github.com/chud-lori/go-boilerplate/grpc_service"
...
func TestSendGrpcMailWithOpts(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		err := grpc_service.SendGrpcMailWithOpts("test@example.com", "hello", grpc.WithContextDialer(bufDialer), grpc.WithInsecure())
		require.NoError(t, err)
	})

	t.Run("missing email triggers error", func(t *testing.T) {
		err := grpc_service.SendGrpcMailWithOpts("", "hello", grpc.WithContextDialer(bufDialer), grpc.WithInsecure())
		require.Error(t, err)
	})
}
